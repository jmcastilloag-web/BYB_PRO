...
window.render = () => {
    ...
    if (window.vistaActual === "dashboard") {
        ...
        window.data.forEach((d, i) => {
            ...
            const obs = (k) => {
                const meta = d.observaciones_meta?.[k];
                const badge = meta ? `<span style="font-size:0.72em;color:#1a6ba0;background:#e8f4fd;border:1px solid #b0d4f0;border-radius:4px;padding:1px 6px;margin-left:6px;white-space:nowrap;">👤 ${meta.nombre} · ${meta.ts||''}</span>` : '';
                return `<div style="margin-bottom:8px;"><label style="font-size:0.78em;color:#555;font-weight:600;">📝 Observaciones${badge}</label><textarea id="obs_${k}_${i}" onchange="window.guardarObs(${i},'${k}')" placeholder="Notas..." style="width:100%;min-height:60px;padding:6px 8px;border:1px solid #dde1e7;border-radius:5px;font-size:0.88em;resize:vertical;">${d.observaciones?.[k]||''}</textarea></div>`;
            };
