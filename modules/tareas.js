...
window.agregarTemp = (i) => {
    ...
    window.data[i].temp_registros.push({ t, lc, ll, est, usuario: f.nombre, ts: f.ts });
    window.data[i].temp_registros.sort((a,b) => +a.t - +b.t);
    window.save();
    // Helper fila temperatura
    const _filaTemp = (r, ri, idx) => `
        <tr style="background:${ri%2===0?'#f8fbff':'white'};border-bottom:1px solid #dde1e7;">
            ...
            <td style="padding:3px 8px;font-size:0.72em;color:#1a6ba0;white-space:nowrap;">
                ${r.usuario ? `👤 ${r.usuario}` : ''}
                ${r.ts ? `<br><span style="color:#888;">${r.ts}</span>` : ''}
            </td>
            <td style="padding:3px 4px;text-align:center;"><button onclick="window.quitarTemp(${idx},${ri})" style="background:none;border:none;color:#e74c3c;cursor:pointer;">✕</button></td>
        </tr>`;
    // Actualizar tabla sin re-render completo
    const tbody = document.querySelector(`#temp_tbody_${i}`);
    if (tbody) {
        tbody.innerHTML = window.data[i].temp_registros.map((r,ri) => _filaTemp(r,ri,i)).join('');
    }
...
// Fila reutilizable para quitarTemp
window._filaTemp = (r, ri, idx) => `
    <tr style="background:${ri%2===0?'#f8fbff':'white'};border-bottom:1px solid #dde1e7;">
        <td style="padding:3px 8px;text-align:center;font-weight:600;">${r.t}'</td>
        <td style="padding:3px 8px;text-align:center;color:#e74c3c;">${r.lc}°</td>
        <td style="padding:3px 8px;text-align:center;color:#3498db;">${r.ll}°</td>
...
window.quitarTemp = (i, ri) => {
    ...
    const tbody = document.querySelector(`#temp_tbody_${i}`);
    if (tbody) {
        tbody.innerHTML = window.data[i].temp_registros.map((r,ri2) => window._filaTemp(r,ri2,i)).join('');
    }
    setTimeout(() => window.dibujarGraficoTemp(i), 80);
};
...
window.guardarObs = (i, key) => {
    ...
    if (!window.data[i].observaciones_meta) window.data[i].observaciones_meta = {};
    const valor = txt ? txt.value : '';
    window.data[i].observaciones[key]      = valor;
    window.data[i].observaciones_meta[key] = valor.trim() ? window._firma() : null;
    window.save();
};
...
window.guardarCheckDesarme = (i, clave, valor) => {
    ...
    if (!window.data[i].check_desarme_meta) window.data[i].check_desarme_meta = {};
    window.data[i].check_desarme[clave]      = valor;
    window.data[i].check_desarme_meta[clave] = window._firma();
    window.save();
};
...
window.toggleCheckMantencion = (i, clave, checked) => {
    ...
    if (!window.data[i].check_mantencion_meta) window.data[i].check_mantencion_meta = {};
    window.data[i].check_mantencion[clave]      = checked;
    const f = window._firma();
    window.data[i].check_mantencion_resp[clave] = checked ? f.nombre : '';
    window.data[i].check_mantencion_meta[clave] = checked ? f : null;
    window.save();
};
...
window.toggleCheckArmado = (i, clave, checked) => {
    ...
    if (!window.data[i].check_armado_meta) window.data[i].check_armado_meta = {};
    const f = window._firma();
    window.data[i].check_armado[clave]      = checked;
    window.data[i].check_armado_resp[clave] = checked ? f.nombre : '';
    window.data[i].check_armado_meta[clave] = checked ? f : null;
    window.save();
};
...
// ── Instalación de rodamientos con firma ─────────────────────
window._marcarRodamiento = (i, ri, checked) => {
    if (!window.data[i].rodamientos_ok)      window.data[i].rodamientos_ok      = {};
    if (!window.data[i].rodamientos_ok_meta) window.data[i].rodamientos_ok_meta = {};
    window.data[i].rodamientos_ok[ri]      = checked;
    window.data[i].rodamientos_ok_meta[ri] = checked ? window._firma() : null;
    window.save();
    window.render();
};

// ── Check detalle calidad con firma ──────────────────────────
window._marcarDetalle = (i, key, checked) => {
    if (!window.data[i].detalle)      window.data[i].detalle      = {};
    if (!window.data[i].detalle_meta) window.data[i].detalle_meta = {};
    window.data[i].detalle[key]      = checked;
    window.data[i].detalle_meta[key] = checked ? window._firma() : null;
    // Actualizar clase visual sin re-render
    const lbl = document.querySelector(`input[onchange*="_marcarDetalle"][onchange*="'${key}'"]`)?.closest('label');
    if (lbl) lbl.classList.toggle('det-chk-on', checked);
    window.save();
};
...
window.eliminarFotoComponente = (i, etapa, clave, fi) => {
    ...
    const fotoMetaKey = 'fotos_b64_' + etapa + '_meta';
    if (!window.data[i][fotoB64Key]?.[clave]) return;
    window.data[i][fotoB64Key][clave].splice(fi, 1);
    if (window.data[i][fotoMetaKey]?.[clave]) {
        window.data[i][fotoMetaKey][clave].splice(fi, 1);
    }
    window.save();
    window.render();
};
