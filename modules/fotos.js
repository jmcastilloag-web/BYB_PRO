...
window.subirFotosComponente = async (i, etapa, clave, inputEl) => {
    ...
    const keyM   = 'fotos_b64_' + etapa + '_meta';   // ← meta por componente

    if (!d[keyP])        d[keyP] = {};
    if (!d[keyP][clave]) d[keyP][clave] = [];
    if (!d[keyM])        d[keyM] = {};
    if (!d[keyM][clave]) d[keyM][clave] = [];
    ...
    const aSubir = Array.from(files).slice(0, disponibles);
    const firma  = window._firma ? window._firma() : { nombre: '—', usuario: '—', ts: '' };
    let subidas  = 0;
    ...
    for (let fi = 0; fi < aSubir.length; fi++) {
        ...
        try {
            ...
            d[keyM][clave].push({ usuario: firma.nombre, ts: firma.ts });  // ← firma
            subidas++;
        } catch (e) {
...
window._htmlFotosComponente = (i, etapa, clave, fotos, fotosMeta) => {
    ...
    fotos.forEach((item, fi) => {
        ...
        const thumb = item.url
            ...
            : src;
        const meta = fotosMeta ? (fotosMeta[fi] || null) : null;
        html += `<div style="position:relative;display:inline-block;text-align:center;">
            <a href="${src}" target="_blank">
                <img src="${thumb}" style="width:54px;height:54px;object-fit:cover;border-radius:4px;border:1.5px solid #b0c8e8;cursor:pointer;display:block;" loading="lazy" onerror="this.src='${src}'">
            </a>
            ${meta ? `<span style="display:block;font-size:0.62em;color:#1a6ba0;white-space:nowrap;max-width:54px;overflow:hidden;text-overflow:ellipsis;" title="${meta.usuario||''} · ${meta.ts||''}">👤 ${meta.usuario||''}</span>` : ''}
