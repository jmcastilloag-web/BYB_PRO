...
// ¿Qué cambiar aquí?
//   - Checklist de componentes del desarme
//   - Tareas de mantención
//   - Botones de avance de flujo
//   - Observaciones y fotos del área
// ============================================================

window.renderAreaDesarme = (d, i, obs) => {
    let UI = "";
    ...
    if (d.estado === 'desarme') {
            ...
            <div style="background:#fff8f0;border:1px solid #f0c080;border-radius:6px;padding:10px;margin-bottom:10px;">
                ...
                <div id="des_lista_${i}">
                    ${(d.hallazgos_lista||[]).map((item,hi) => {
                        const meta = (d.hallazgos_lista_meta||[])[hi];
                        return `
                        <div style="display:flex;align-items:center;gap:6px;padding:5px 0;border-bottom:1px solid #f0e0c0;">
                            <span style="flex:1;font-size:0.88em;">• ${item}
                                ${meta ? `<span style="font-size:0.7em;color:#1a6ba0;background:#e8f4fd;border:1px solid #b0d4f0;border-radius:3px;padding:1px 4px;margin-left:4px;">👤 ${meta.nombre} · ${meta.ts||''}</span>` : ''}
                            </span>
            ...
            <div style="background:#f0f4ff;border:1px solid #c0d0f0;border-radius:6px;padding:10px;margin-bottom:10px;">
                ...
                <div id="tarea_desarme_lista_${i}">
                    ${(d.tareas_desarme||[]).map((item,ti) => {
                        const meta    = (d.tareas_desarme_meta||[])[ti] || {};
                        const chk     = (d.tareas_desarme_checks||{})[ti];
                        const chkMeta = (d.tareas_desarme_checks_meta||{})[ti];
                        return `
                        <div style="display:flex;align-items:flex-start;gap:6px;padding:5px 0;border-bottom:1px solid #d0dcf8;">
                            <label style="flex:1;display:flex;align-items:flex-start;gap:6px;cursor:pointer;">
                                <input type="checkbox" ${chk?'checked':''} style="margin-top:2px;"
                                    onchange="window._marcarTarea('desarme',${i},${ti},this.checked);">
                                <span style="font-size:0.87em;" class="${chk?'tachado':''}">${item}
            ...
            <div style="background:#f4f8ff;border:1.5px solid #b0c8e8;border-radius:8px;padding:12px 14px;margin-bottom:14px;overflow-x:auto;">
                ...
                <table style="width:100%;border-collapse:collapse;font-size:0.82em;">
                    <thead><tr style="background:#004F88;color:white;">
                        ...
                        <th style="padding:6px 10px;text-align:left;min-width:120px;">👤 Técnico</th>
                    ...
                    ${(window.ITEMS_CHECK_DESARME||[]).map((item, ci) => {
                        ...
                        const meta  = (d.check_desarme_meta||{})[item.k];
                        const obsV  = (d.check_desarme_obs||{})[item.k] || '';
                        const fotos = ((d.fotos_b64_desarme||{})[item.k]) || [];
                        const fotosMeta = ((d.fotos_b64_desarme_meta||{})[item.k]) || [];
                        ...
                        return `<tr style="background:${rowBg};border-bottom:1px solid #dde1e7;">
                            ...
                            <td style="padding:5px 8px;">
                                ${meta ? '<span style="font-size:0.72em;color:#1a6ba0;white-space:nowrap;">👤 '+meta.nombre+'<br><span style="color:#888;">'+(meta.ts||'')+'</span></span>' : '<span style="color:#ccc;font-size:0.78em;">—</span>'}
                            </td>
                            <td style="padding:4px 8px;">
                                ${window._htmlFotosComponente ? window._htmlFotosComponente(i,'desarme',item.k,fotos,fotosMeta) : ''}
                                ${fotos.length < 10
                                    ? `<label id="btn_foto_desarme_${i}_${item.k}" style="display:inline-flex;align-items:center;gap:4px;margin-top:4px;background:#e8f0fe;border:1px solid #b0c8e8;border-radius:4px;padding:3px 8px;cursor:pointer;font-size:0.8em;color:#004F88;font-weight:600;">
                                        📷 ${fotos.length > 0 ? fotos.length+'/10' : 'Fotos'}
                                        <input type="file" accept="image/*" multiple style="display:none;"
                                            onchange="window.subirFotosComponente(${i},'desarme','${item.k}',this)">
                                    </label> ${window._btnCamaraComponente ? window._btnCamaraComponente(i,'desarme',item.k) : ''}`
        ...
        const _chkMMeta= d.check_mantencion_meta || {};
        const _fotosM  = d.fotos_b64_mantencion || {};
        const _fotosMeta = d.fotos_b64_mantencion_meta || {};
               ...
               <div style="background:#f8fbff;border:1.5px solid #b0c8e8;border-radius:8px;padding:10px 14px;margin-bottom:14px;overflow-x:auto;">
                   ...
                   <table style="width:100%;border-collapse:collapse;font-size:0.82em;">
                       ...
                       ${_itemsMant.map((it, ci2) => {
                           ...
                           const obsM   = (_chkMObs[it.k]||'').split('"').join('&quot;');
                           const meta   = _chkMMeta[it.k];
                           const fotos  = _fotosM[it.k] || [];
                           const fotosMeta = _fotosMeta[it.k] || [];
                           ...
                           return `<tr style="background:${rowBg};border-bottom:1px solid #dde1e7;">
                               ...
                               <td style="padding:4px 8px;">
                                   ${(window._htmlFotosComponente||function(){return '';})(i,'mantencion',it.k,fotos,fotosMeta)}
                                   ${fotos.length < 10
            ...
            <div style="background:#f0f4ff;border:1px solid #c0d0f0;border-radius:6px;padding:10px;margin-bottom:10px;">
                ...
                <div id="tarea_mantencion_lista_${i}">
                    ${(d.tareas_mantencion||[]).map((item,ti) => {
                        const meta    = (d.tareas_mantencion_meta||[])[ti] || {};
                        const chk     = (d.tareas_mantencion_checks||{})[ti];
                        const chkMeta = (d.tareas_mantencion_checks_meta||{})[ti];
                        return `
                        <div style="display:flex;align-items:flex-start;gap:6px;padding:5px 0;border-bottom:1px solid #d0dcf8;">
                            <label style="flex:1;display:flex;align-items:flex-start;gap:6px;cursor:pointer;">
                                <input type="checkbox" ${chk?'checked':''} style="margin-top:2px;"
                                    onchange="window._marcarTarea('mantencion',${i},${ti},this.checked);">
                                <span style="font-size:0.87em;" class="${chk?'tachado':''}">${item}
                                    ${meta.nombre ? `<span style="font-size:0.72em;color:#1a6ba0;background:#e8f4fd;border:1px solid #b0d4f0;border-radius:4px;padding:1px 5px;margin-left:4px;">👤 ${meta.nombre} · ${meta.ts||''}</span>` : ''}
                                    ${chk && chkMeta ? `<span style="font-size:0.7em;color:#27ae60;margin-left:4px;">✅ ${chkMeta.nombre} · ${chkMeta.ts||''}</span>` : ''}
                                </span>
                            </label>
