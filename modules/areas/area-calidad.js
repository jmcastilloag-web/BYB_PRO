...
window.renderAreaCalidad = (d, i, obs) => {
    ...
    if (d.estado === 'ingresos_pendientes') {
                                    ...
                                    <div style="display:flex;align-items:center;gap:6px;padding:4px 0;border-bottom:1px solid #d0dcf8;">
                                        <label style="flex:1;display:flex;align-items:center;gap:6px;cursor:pointer;">
                                            <input type="checkbox" ${(d.tareas_calidad_checks||{})[ti]?'checked':''}
                                                ...
                                                onchange="window._marcarTarea('calidad',${i},${ti},this.checked);">
                                            <span style="font-size:0.87em;${(d.tareas_calidad_checks||{})[ti]?'text-decoration:line-through;color:#888;':''}">
                                                ${item}
                                                ${(d.tareas_calidad_meta||[])[ti]?.nombre ? `<span style="font-size:0.7em;color:#1a6ba0;background:#e8f4fd;border:1px solid #b0d4f0;border-radius:3px;padding:1px 4px;margin-left:3px;">👤 ${(d.tareas_calidad_meta||[])[ti].nombre} · ${(d.tareas_calidad_meta||[])[ti].ts||''}</span>` : ''}
                                                ${(d.tareas_calidad_checks||{})[ti] && (d.tareas_calidad_checks_meta||{})[ti] ? `<span style="font-size:0.7em;color:#27ae60;margin-left:3px;">✅ ${(d.tareas_calidad_checks_meta||{})[ti].nombre} · ${(d.tareas_calidad_checks_meta||{})[ti].ts||''}</span>` : ''}
                                            </span>
                                        </label>
                    ...
                    const _detMeta = d.detalle_meta || {};
                    const chk = (key, label) => {
                        const m = _detMeta[key];
                        return `
                        <label class="det-chk-label ${det[key]?'det-chk-on':''}">
                            <input type="checkbox" ${det[key]?'checked':''} onchange="window._marcarDetalle(${i},'${key}',this.checked);">
                            <span>${label}${m ? ` <span style="font-size:0.68em;color:#1a6ba0;background:#e8f4fd;border:1px solid #b0d4f0;border-radius:3px;padding:1px 4px;margin-left:3px;">👤 ${m.nombre} · ${m.ts||''}</span>` : ''}</span>
                        </label>`; };
                    UI = `
                        ${hallazgos.length > 0 ? `
                    ...
                    UI = `<h3>Pruebas Dinámicas</h3>
                        ...
                        <div style="background:#f0f8ff;border:1px solid #b0d4f0;border-radius:6px;padding:10px;margin-bottom:10px;">
                            <div style="display:grid;grid-template-columns:auto 1fr 1fr 1fr 1fr auto;gap:6px;align-items:center;margin-bottom:8px;">
                                ...
                                <span style="font-size:0.8em;font-weight:700;color:#004F88;text-align:center;">TÉCNICO</span>
                                <span></span>
                                    ...
                                    <div style="display:flex;align-items:center;gap:6px;padding:4px 0;border-bottom:1px solid #d0dcf8;">
                                        <label style="flex:1;display:flex;align-items:center;gap:6px;cursor:pointer;">
                                            <input type="checkbox" ${(d.tareas_pruebas_checks||{})[ti]?'checked':''}
                                                style="margin-top:2px;"
                                                onchange="window._marcarTarea('pruebas',${i},${ti},this.checked);">
                                            <span style="font-size:0.87em;${(d.tareas_pruebas_checks||{})[ti]?'text-decoration:line-through;color:#888;':''}">
                                                ${item}
                                                ${(d.tareas_pruebas_meta||[])[ti]?.nombre ? `<span style="font-size:0.7em;color:#1a6ba0;background:#e8f4fd;border:1px solid #b0d4f0;border-radius:3px;padding:1px 4px;margin-left:3px;">👤 ${(d.tareas_pruebas_meta||[])[ti].nombre} · ${(d.tareas_pruebas_meta||[])[ti].ts||''}</span>` : ''}
                                                ${(d.tareas_pruebas_checks||{})[ti] && (d.tareas_pruebas_checks_meta||{})[ti] ? `<span style="font-size:0.7em;color:#27ae60;margin-left:3px;">✅ ${(d.tareas_pruebas_checks_meta||{})[ti].nombre} · ${(d.tareas_pruebas_checks_meta||{})[ti].ts||''}</span>` : ''}
                                            </span>
                                        </label>
