...
// ── Fotos de secciones (mediciones, pruebas) ────────────────
// Sube fotos a una clave directa del objeto data (array de {b64,ext})
window.subirFotosSeccion = async (i, fotoKey, inputEl) => {
    const files = Array.from(inputEl.files);
    if (!files.length) return;
    const d = window.data[i];
    if (!d[fotoKey]) d[fotoKey] = [];
    const actuales = d[fotoKey].length;
    const disponibles = 10 - actuales;
    if (disponibles <= 0) { alert('Ya tienes 10 fotos en esta sección.'); return; }
    const aSubir = files.slice(0, disponibles);
    try {
        for (const file of aSubir) {
            const b64 = await _fileToBase64(file);
            const ext = file.name.split('.').pop().toLowerCase();
            d[fotoKey].push({ b64, ext: ext === 'jpg' ? 'jpeg' : ext });
        }
        window.save();
        window.render();
    } catch(e) { alert('Error al procesar foto: ' + e.message); }
};
window.eliminarFotoSeccion = (i, fotoKey, fi) => {
    if (!window.data[i][fotoKey]) return;
    window.data[i][fotoKey].splice(fi, 1);
    window.save();
    window.render();
};
// Genera miniaturas HTML para un array de {b64,ext}
window._htmlFotosB64 = (i, fotoKey, fotos) => {
    if (!fotos || fotos.length === 0) return '';
    let html = '<div style="display:flex;flex-wrap:wrap;gap:4px;margin-top:4px;">';
    fotos.forEach(function(item, fi) {
        const src = (item && item.b64) ? 'data:image/' + (item.ext||'jpeg') + ';base64,' + item.b64 : '';
        if (!src) return;
        html += '<div style="position:relative;display:inline-block;">';
        html += '<a href="' + src + '" target="_blank"><img src="' + src + '" style="width:54px;height:54px;object-fit:cover;border-radius:4px;border:1.5px solid #b0c8e8;cursor:pointer;"></a>';
        html += '<button onclick="window.eliminarFotoSeccion(' + i + ',&quot;' + fotoKey + '&quot;,' + fi + ')" style="position:absolute;top:-4px;right:-4px;background:#e74c3c;color:white;border:none;border-radius:50%;width:16px;height:16px;font-size:9px;cursor:pointer;line-height:16px;padding:0;text-align:center;">✕</button>';
        html += '</div>';
    });
    html += '</div>';
    return html;
...
// Fotos para tarjetas de mecánica
window.subirFotosMec = async (i, clave, inputEl) => {
    const files = Array.from(inputEl.files);
    if (!files.length) return;
    const d = window.data[i];
    if (!d.mec_trab_usuario) d.mec_trab_usuario = {};
    if (!d.mec_trab_usuario[clave]) d.mec_trab_usuario[clave] = {usuario:'',medidas:'',fotos_b64:[],ok:false};
    if (!d.mec_trab_usuario[clave].fotos_b64) d.mec_trab_usuario[clave].fotos_b64 = [];
    const actuales = d.mec_trab_usuario[clave].fotos_b64.length;
    const disponibles = 10 - actuales;
    if (disponibles <= 0) { alert('Ya tienes 10 fotos.'); return; }
    try {
        for (const file of files.slice(0, disponibles)) {
            const b64 = await _fileToBase64(file);
            const ext = file.name.split('.').pop().toLowerCase();
            d.mec_trab_usuario[clave].fotos_b64.push({ b64, ext: ext==='jpg'?'jpeg':ext });
        }
        window.save(); window.render();
